...
def parse_splatfest_results(festival_id=None):
    ...
    print("\nSplatfest ID {} was between {} and {}.".format(results["festivals"][0]["festival_id"],
                                                            Fore.GREEN + alpha_name + Style.RESET_ALL,
    ...
    print("This Splatfest's winner was {}Team {}.".format(Fore.YELLOW + Style.BRIGHT, winner + Style.RESET_ALL))
...
def parse_schedules(schedule, splatfest_active, rotation=0):
    ...
    if rotation == 0:
        print("{:<18}{:<17}{} and {}".format(schedule["regular"][0]["game_mode"]["name"],
        ...
        print("{:<18}{:<17}{} and {}".format(schedule["gachi"][0]["game_mode"]["name"],
        ...
        print("{:<18}{:<17}{} and {}".format(schedule["league"][0]["game_mode"]["name"],
        ...
        print("{:<18}{:<17}{} and {}".format(schedule["gachi"][rotation]["game_mode"]["name"],
        ...
        print("{:<18}{:<17}{} and {}".format(schedule["league"][rotation]["game_mode"]["name"],
    ...
    for i in range(len(colors["festivals"])):
        if len(colors["festivals"]) > 1:
            print("\nSplatfest #{} [{}] ({} vs {})".format(len(colors["festivals"]) - i, Fore.BLUE + Style.BRIGHT + str(
                colors["festivals"][i]["festival_id"]) + Style.RESET_ALL,
                                                           colors["festivals"][i]["names"]["alpha_short"],
                                                           colors["festivals"][i]["names"]["bravo_short"]))
        else:
            print("\nLatest Splatfest [{}] ({} vs {})".format(
                Fore.BLUE + Style.BRIGHT + str(colors["festivals"][i]["festival_id"]) + Style.RESET_ALL,
                colors["festivals"][i]["names"]["alpha_short"], colors["festivals"][i]["names"]["bravo_short"]))

        for j in ["alpha", "bravo", "middle"]:
            print("{}'s colors:".format(
                Fore.GREEN + colors["festivals"][i]["names"].get(j + "_short", "Neutral") + Style.RESET_ALL))
            print("{}, {}, {}".format(int(round(float(colors["festivals"][i]["colors"][j]["r"]) * 255)),
                                      int(round(float(colors["festivals"][i]["colors"][j]["g"]) * 255)),
                                      int(round(float(colors["festivals"][i]["colors"][j]["b"]) * 255))))
            print("#{:02X}{:02X}{:02X}".format(int(round(float(colors["festivals"][i]["colors"][j]["r"]) * 255)),
                                               int(round(float(colors["festivals"][i]["colors"][j]["g"]) * 255)),
                                               int(round(float(colors["festivals"][i]["colors"][j]["b"]) * 255))))
...
def parse_league_stats(records):
    ...
    medals = {"gold_count": "gold", "silver_count": "silver", "bronze_count": "bronze", "no_medal_count": "no"}

    print("\nYou've earned these medals from playing League:")
    print("\tTeams\tPairs")
    print("Gold\t{:<8}{}".format(league_data["team"]["gold_count"], league_data["pair"]["gold_count"]))
    print("Silver\t{:<8}{}".format(league_data["team"]["silver_count"], league_data["pair"]["bronze_count"]))
    print("Bronze\t{:<8}{}".format(league_data["team"]["bronze_count"], league_data["pair"]["bronze_count"]))
